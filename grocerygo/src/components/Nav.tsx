...
function Nav({ user }: { user: IUser }) {
  ...
  const sideBar = (mounted && menuOpen && typeof document !== 'undefined') ? createPortal(
    <AnimatePresence>
      <motion.div
      ...
      >
        ...
        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/20 transition-all shadow-inner w-full'>
          <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg shrink-0'>
            {user?.image ? <Image src={user.image} alt='user' fill className='object-cover rounded-full' /> : <User className='w-full h-full p-2' />}
          </div>
          <div>
        ...
        <div className='flex flex-col gap-3 font-medium mt-6'>
          <Link href={"/admin/add-grocery"} onClick={() => setMenuOpen(false)} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all w-full'>
            <PlusCircle className='w-5 h-5' />Add Grocery
          </Link>
          <Link href={"/admin/view-grocery"} onClick={() => setMenuOpen(false)} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all w-full'>
            <BoxesIcon className='w-5 h-5' />View Grocery
          </Link>
          <Link href={"/admin/manage-orders"} onClick={() => setMenuOpen(false)} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all w-full'>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
        {/* Search icon on mobile - shown for both user and admin */}
        {(user?.role === "user" || user?.role === "admin") && (
          <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center shadow-md hover:scale-105 transition md:hidden cursor-pointer' onClick={() => setSearchBarOpen((prev) => !prev)}>
            <Search className='text-green-600 w-6 h-6' />
          </div>
        )}

        {/* Cart icon - shown for both user and admin */}
        {(user?.role === "user" || user?.role === "admin") && (
        ...
        {user?.role === "admin" && (
          <>
            <div className='hidden md:flex items-center gap-4'>
              ...
              <Link href={"/admin/view-grocery"} className='flex items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'><BoxesIcon className='w-5 h-5' />View Grocery</Link>
            ...
            <div className='md:hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md cursor-pointer' onClick={() => setMenuOpen(prev => !prev)}>
        ...
        <div className='relative' ref={profileDropDown}>
          <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform relative cursor-pointer' onClick={() => setOpen(prev => !prev)}>
            {user?.image ? <Image src={user.image} alt='user' fill className='object-cover rounded-full' /> : <User className='text-gray-600' />}
          </div>
          <AnimatePresence>
            {open && (
              <motion.div
              ...
              >
                <div className='flex items-center gap-3 px-3 py-2 border-b border-gray-100'>
                  <div className='relative w-10 h-10 rounded-full bg-green-100 flex items-center justify-center overflow-hidden shrink-0'>
                    {user?.image ? <Image src={user.image} alt='user' fill className='object-cover rounded-full' /> : <User className='text-gray-600' />}
                  </div>
          ...
          <AnimatePresence>
            {searchbarOpen && (
              ...
              >
                ...
                <button onClick={() => setSearchBarOpen(false)} aria-label="Close search">
