...
function UserOrderCard({ order }: { order: IOrder }) {
  ...
  return (
    ...
    >
      <div
        className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-gradient-to-r from-green-50 to-white"
      >
        <div>
      ...
      <div className="p-5 space-y-4">
        ...
        {status !== "delivered" && (
          <button className='w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2.5 rounded-xl shadow hover:bg-green-700 transition cursor-pointer my-3' onClick={() => router.push(`/user/track-order/${order._id?.toString()}`)}><Truck size={16} />Track Your Order</button>
        )}
        ...
        <div className='border-t pt-3 flex justify-between items-center text-sm font-semibold text-gray-800'>
          <div className='flex items-center gap-2 text-gray-700 text-sm'>
            <Truck size={16} className='text-green-600' />
            <span >Delivery: <span className='text-green-700 font-semibold'>
              {status}</span></span>
          </div>
          <div>
